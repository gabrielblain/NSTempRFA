test_that("Add_RegProb returns probabilities between 0.0001 and 0.9999", {
  ...
  reg_par <- data.frame(matrix(c(40, 0.1, -0.001, 2, 0.05, 0.1), nrow = 1))
  site_temp <- rnorm(100, mean = 40, sd = 2)
  n.year <- 50

  result <- Add_RegProb(quantiles, reg_par, site_temp, n.year)
...
test_that("Add_RegProb throws error with missing quantiles", {
  ...
  expect_error(Add_RegProb(quantiles, reg_par, site_temp, n.year),
               regexp = "must be a numeric vector with no missing values")
})

test_that("Add_RegProb throws error for invalid reg_par dimensions", {
  quantiles <- c(39.5, 40.0, 40.5)
  wrong_reg_par <- data.frame(matrix(c(40, 0.1, -0.001, 2, 0.05), nrow = 1))  # Only 5 columns
  site_temp <- rnorm(100, mean = 40, sd = 2)
  n.year <- 30

  expect_error(Add_RegProb(quantiles, wrong_reg_par, site_temp, n.year),
               regexp = "must be a 1-row, 6-column data.frame")
})
...
test_that("Add_RegProb throws error if scale parameter becomes non-positive", {
  quantiles <- c(39.5, 40.0, 40.5)
  reg_par <- data.frame(matrix(c(40, 0.1, -0.001, -2, -0.05, 0.1), nrow = 1))  # Negative scale intercept and slope
  site_temp <- rnorm(100, mean = 40, sd = 2)
  n.year <- 30
