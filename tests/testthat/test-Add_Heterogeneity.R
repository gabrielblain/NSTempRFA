test_that("Add_Heterogeneity returns a numeric H value", {
  ...
  result <- Add_Heterogeneity(dataset.add = dataset.add, rho = 0.5, Ns = 100)
...
test_that("Add_Heterogeneity throws error when a site has fewer than 10 observations", {
  dataset.add <- matrix(rnorm(300), ncol = 2)
  expect_error(Add_Heterogeneity(dataset.add, rho = 0.5, Ns = 100),
...
test_that("Add_Heterogeneity throws error for invalid rho values", {
  dataset.add <- matrix(rnorm(1500), ncol = 15)
  expect_error(
...
test_that("Add_Heterogeneity throws error if Ns is too small", {
  dataset.add <- matrix(rnorm(1500), ncol = 15)
  expect_error(Add_Heterogeneity(dataset.add, rho = 0.5, Ns = 50),
               "Ns should be larger than 99")
})

test_that("Add_Heterogeneity handles fallback when Kappa fails", {
  set.seed(123)
  dataset.add <- matrix(rnorm(1500, mean = 1, sd = 0.0001), ncol = 15)  # Near-constant but not fully
  expect_error(Add_Heterogeneity(dataset.add, rho = 0.5, Ns = 100), NA)
})
