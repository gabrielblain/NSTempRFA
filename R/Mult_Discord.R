#' Discordance measure calculated from the multiplicative approach
#'
#' This function calculates the Hosking and Wallis' discordance measure using the (original)
#' multiplicative approach, as proposed in Hosking and Wallis (1997) <doi:10.1017/CBO9780511529443>.
...
#'   between the site's L-moment ratios and the regional average, based on the multiplicative
...
#'
#' dataset <- dataset
#' d <- Mult_Discord(dataset)
#'
Mult_Discord <- function(dataset) {
  n <- ncol(dataset)
  n.sites <- n - 1
  d <- as.data.frame(matrix(NA, n.sites, 8))
  dataset.year <- dataset[, 2:n]
  d[, 1:7] <- regsamlmu(dataset.year)
  d[, 8] <- sqrt(getDistance(Cov(d[, 4:6])))

  colnames(d) <- c("Local", "SampleSize", "l_1", "t", "t_3", "t_4", "t_5", "discord")
  return(d)
}
