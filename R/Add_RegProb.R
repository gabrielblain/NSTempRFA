...
#' @param reg_par A 6-column and 1-row `data.frame` as that generated by `reg_par()`.
...
Add_RegProb <- function(quantiles, reg_par, site_temp, n.year) {
  # Input checks
  if (!is.numeric(quantiles) || any(is.na(quantiles))) {
    stop("`quantiles` must be a numeric vector with no missing values.")
  }

  if (!is.data.frame(reg_par) || ncol(reg_par) != 6 || nrow(reg_par) != 1) {
    stop("`reg_par` must be a 1-row, 6-column data.frame as returned by `reg_par()`.")
  }
