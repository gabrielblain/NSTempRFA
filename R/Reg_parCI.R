...
Reg_parCI <- function(temperatures,reg_par,n.boots){
  ...
  # Preallocate matrix
  IDD.series.boot <- matrix(NA_real_, all.lines, n.stations)
  ...
  for (r in seq_len(n.boots)) {
    ...
    find.best.boot <- best_model(back.orig)
    reg_par.overall.boot[r, ] <- as.matrix(reg_par(best_model=find.best.boot$atsite.models))
