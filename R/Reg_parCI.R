...
#' O’Brien and Burn (2014) <10.1016/j.jhydrol.2014.09.041>.
#' @return
...
Reg_parCI <- function(add_data,model,reg_par,n.boots){
  ...
  if (!is.data.frame(reg_par) || !all(sapply(reg_par, is.numeric))) {
    stop("Input 'reg_par' must be a numeric data frame with 6 columns.")
  }
  if (ncol(reg_par) != 6 || nrow(reg_par) != 1) {
    stop("Input 'reg_par' must have exactly 1 row and 6 columns.")
  }
  ...
  reg_par.overall.boot <- matrix(NA_real_, n.boots, 6)
  ...
  colnames(CI_matrix) <- c("weighted_mu0", "weighted_mu1", "weighted_mu2", "weighted_sigma0", "weighted_sigma1", "weighted_shape")
  # Print results
  return(CI_matrix)
}
