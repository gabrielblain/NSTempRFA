...
Reg_parCI <- function(add_data, model, reg_par, n.boots = 999) {
  ...
  if (!is.numeric(add_data)) {
    stop("Input 'add_data' must be numeric.")
  }
  
  # Validate model parameter
  if (!is.numeric(model) || model != as.integer(model) || model < 1 || model > 4) {
    stop("Parameter 'model' must be an integer between 1 and 4.")
  }
  ...
  # Calculate temporal parameters and transform data
